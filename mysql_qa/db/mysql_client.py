...
import pymysql
import pandas as pd
from mysql_qa.sqlbase import Config
from mysql_qa.sqlbase.logger import logger
...
class MySQLClient:
    ...
    def insert_data(self, csv_path):
        try:
            ...
            self.connection.commit()
            self.cursor.execute("SELECT COUNT(*) FROM jpkb")
            print("表 jpkb 记录数:", self.cursor.fetchone()[0])
